  ...
  init() {
    if (this.location) {
      ...
      if (this.viewType == 'location-view') {
        ...
        markerGpsStr = `-1,http://cache.amap.com/lbs/static/cuntom_marker1.png,0:${this.location[0]},${this.location[1]}`
        ...
        this.mapStaticImg = 'http://restapi.amap.com/v3/staticmap?size=' + size + '&markers=' + markerGpsStr + '&key=' + this.STATIC_MAP_KEY;
      } else if (this.viewType == 'map-locations') {
        //地图坐标下，this.location 为多个lnglat数组
        if (this.location && this.location.length) {
          let markerList = [];
          for (let i = 0; i < this.location.length && i < 50; i++) {
            markerList.push('mid,' + color + ',' + i + ':' + this.location[i].lnglat[0] + ',' + this.location[i].lnglat[1]);
          }
